...
export const vue = (): IPlugin => {
  const namespace = 'vue'
  return {
    name: namespace,
    bundleStart: (options, helper) => {
      const { isAbsolute, join, relative, rootName, dirname, basename } =
        helper.getPathUtils()
      const vfs = helper.getVirtualFileSystem()
      const { vueCompiler: compiler } = helper.getBuildOptions()
      const vuePlugin: EsbuildPlugin = {
        name: namespace,
        setup (build) {
          ...
          build.onLoad(
            { filter: /\?vue&type=template/, namespace },
            async args => {
          ...
          build.onLoad(
            { filter: /\?vue&type=script/, namespace },
            async args => {
              const filepath = removeQuery(args.path)
              const source = (await vfs.readFile(filepath)) || ''
          ...
          build.onLoad(
            { filter: /\?vue&type=style/, namespace },
            async args => {
              const filepath = removeQuery(args.path)
              const source = (await vfs.readFile(filepath)) || ''
              const { descriptor } = compiler.parse(source)
